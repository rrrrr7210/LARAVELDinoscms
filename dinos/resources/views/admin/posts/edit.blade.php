...
@section('content')
    <div class="card-body text-center col-md-5 mt-5" id="image_preview">
        <div id="message"></div>
        <img class="mt-5 rounded" id="previewing" src="{{$post->image ? $post->image->image_name : '/images/no.png'}}" width="300" height="240" />
    </div>
    <div class="card-body text-center col-md-6 ml-lg-5">
        <h1>Create Post</h1>
        <div>
            {!! Form::model($post, ['method' => 'PATCH', 'action' => ['AdminPostsController@update', 'id' => $post->id ], 'files' => true]) !!}
            ...
            <div class="form-group d-inline-block">
                {!! Form::submit('Update Post', ['class' => 'btn btn-success m-md-2', 'name' => 'upload']) !!}
            </div>
            {!! Form::close() !!}
